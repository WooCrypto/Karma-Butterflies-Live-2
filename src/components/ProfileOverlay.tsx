          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="bg-[#111111]/90 border border-white/10 rounded-2xl p-5 space-y-4">
                ...
                {isScanningSolana ? (
                  <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-3 text-[11px] text-slate-250 leading-relaxed font-sans space-y-2 animate-pulse">
                    <div className="flex items-center gap-2 text-emerald-400 font-extrabold uppercase font-mono tracking-wider">
                      <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                      <span>SCANNING SOLANA BLOCKCHAIN</span>
                    </div>
                    <p>
                      Scanning token accounts of <code className="text-amber-400 font-mono text-[10px] bg-black/40 px-1 py-0.5 rounded">{address.slice(0, 8)}...</code> for verified Karma Butterfly NFTs launched from smart contract: <br />
                      <span className="text-amber-300 font-mono text-[9px] break-all">FXSVHzLvVFey57U8ETuhHzrzDRT3FhvqzbxWpyoAJA4c</span>
                    </p>
                  </div>
                ) : solanaNftsOwned.length > 0 ? (
                  <div className="bg-emerald-950/20 border border-emerald-500/40 rounded-xl p-3 text-[11px] text-slate-200 leading-relaxed font-sans space-y-1">
                    <div className="flex items-center gap-1.5 text-emerald-400 font-extrabold uppercase font-mono tracking-wider">
                      <Sparkles className="w-3.5 h-3.5 text-amber-400 animate-bounce" />
                      <span>🎉 VERIFIED HOLDER DETECTED</span>
                    </div>
                    <p>
                      Found <strong>{solanaNftsOwned.length} official Karma Butterfly NFT(s)</strong> in this wallet! Your live view butterfly is now synchronized with on-chain metadata from contract <code className="text-[#F59E0B] font-mono text-[10px] bg-black/40 px-1 py-0.5 rounded">{address.slice(0, 4)}...</code>.
                    </p>
                  </div>
                ) : verificationError ? (
                  <div className="bg-rose-950/20 border border-rose-500/30 rounded-xl p-3 text-[11px] text-slate-300 leading-relaxed font-sans space-y-1">
                    <div className="flex items-center gap-1.5 text-rose-400 font-extrabold uppercase font-mono tracking-wider">
                      <span>⚠️ {verificationError}</span>
                    </div>
                    <p>
                      We could not contact the RPC node right now, but your full testing sandbox options are unlocked below for you to freely preview, download, and evolve!
                    </p>
                  </div>
                ) : isConnected && address ? (
                  <div className="bg-[#111111]/80 border border-[#F59E0B]/10 rounded-xl p-3 text-[11px] text-slate-300 leading-relaxed font-sans space-y-1">
                    <div className="flex items-center gap-1.5 text-amber-400 font-extrabold uppercase font-mono tracking-wider">
                      <span>⚠️ SANDBOX WALLET OVERLAY</span>
                    </div>
                    <p>
                      No Karma Butterfly NFT from contract <code className="text-[#F59E0B] font-mono">FXSVHzLv...</code> was detected in this address, but standard simulator controls are active. Try selecting user presets below!
                    </p>
                    <p className="text-[10px] text-[#F59E0B] font-mono bg-amber-500/5 p-1.5 rounded mt-1">
                      💡 Tip: Click "Disconnect" and type <code className="text-white">demo</code> or <code className="text-white">test</code> to experience the dynamic verified holder status.
                    </p>
                  </div>
                ) : (
                  <div className="bg-[#111111]/80 border border-white/5 rounded-xl p-3 text-[11px] text-slate-300 leading-relaxed font-sans space-y-1">
                    <div className="flex items-center gap-1.5 text-[#F59E0B] font-extrabold uppercase font-mono tracking-wider">
                      <span>🦋 ON-CHAIN NFT INTEGRATION</span>
                    </div>
                    <p>
                      Connect your Phantom wallet or paste any Solana address. 
                    </p>
                    <div className="text-[10px] text-slate-400 bg-black/40 p-2 rounded border border-white/5 space-y-1 mt-1 font-mono">
                      <span className="text-amber-400 font-bold uppercase block text-[9.5px]">💡 Sandboxed Preview Tips</span>
                      <p className="leading-tight">
                        • If Phantom does not open, click the "open in new tab" icon in your browser to bypass iframe sandbox restrictions.<br/>
                        • You can type <code className="text-[#34d399]">demo</code> or <code className="text-[#34d399]">test</code> into the manual input box and click Manual to simulate a fully verified Karma NFT Holder!
                      </p>
                    </div>
                  </div>
                )}
                ...
                {isConnected ? (
                  ...
                  <div className="space-y-3">
                    <div className="flex flex-col gap-2">
                      {/* Premium Solana Phantom Wallet Connect Button */}
                      <button
                        onClick={handleConnectPhantom}
                        disabled={isScanningSolana}
                        className="w-full relative px-4 py-3 rounded-xl overflow-hidden bg-gradient-to-r from-[#ab9ff2] via-[#4e44ce] to-[#512da8] text-white font-sans font-black text-xs tracking-wider uppercase hover:opacity-95 transition-all duration-300 cursor-pointer shadow-lg shadow-[#4e44ce]/25 flex items-center justify-center gap-2 border border-white/10 font-extrabold"
                      >
                      ...
                      <div className="flex items-center gap-2 py-1">
                        ...
                        <span className="text-[9px] font-mono text-slate-500 uppercase tracking-wider">or manually input</span>
                        <div className="h-px bg-white/10 flex-1" />
                      </div>
                      ...
                      <div className="flex gap-2">
                        <input
                          type="text"
                          placeholder="Paste address (0x... or Solana Address)"
                          value={address}
                    ...
                    <div className="flex items-center justify-between pt-0.5">
                      ...
                      <span className="text-[9px] text-[#F59E0B] italic block">No wallet? Click Manual to try with any seed.</span>
                    </div>
                  </div>
                )}
